...
def main():
    ...
    print(f"\n[*] API Key Status:")
    print(f"   Gemini API:    {'OK - Configured' if status['gemini'] else 'MISSING - Not set'}")
    print(f"   Replicate API: {'OK - Configured' if status['replicate'] else 'MISSING - Not set'}")

    if not status["gemini"] or not status["replicate"]:
        print(f"\n[!] Some API keys are missing. Edit .env file to add them.")
